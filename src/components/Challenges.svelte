<script>
  ...
  let selected=$state(null),tier=$state("All"),playMode=$state("recommended");
...
<section class="challenges" aria-label="Challenges menu">
  ...
  <fieldset><legend>How do you want to play?</legend><label><input type="radio" bind:group={playMode} value="recommended"/> Recommended</label><label><input type="radio" bind:group={playMode} value="freestyle"/> Freestyle</label><p>{playMode === "recommended" ? "A small set of blocks for this challenge." : "All robot commands and programming blocks. Same goals, more ways to solve them. Shop and inventory stay on your main farm."}</p></fieldset>
    ...
    {#each visible as task}
      <li class:complete={completed.includes(task.id)}>
        ...
        {#if selected===task.id}<div class="details" id={task.id+"-details"} transition:slide={{duration:reduced?0:200}}>
          ...
          {#if !challengeEntryAllowed(challengeReady,practiceTasks.includes(task.id),playMode)}<p class="locked" role="status">Keep farming a little longer to start this new challenge.</p>{/if}
          <button class="enter" disabled={!challengeEntryAllowed(challengeReady,practiceTasks.includes(task.id),playMode)} onclick={()=>onChallenge(task,playMode)}>{practiceTasks.includes(task.id)?"Practice again":"Accept challenge"}</button>
