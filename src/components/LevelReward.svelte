<script>
  ...
  function claimReward(level) {
    ...
    const {
      ...
      item = null,
      amount,
    } = LEVEL_REWARDS_DATA[level] ?? LEVEL_REWARDS_DATA.DEFAULT;
...
{#if Modals[ModalTypes.LEVEL_REWARDS]}
  ...
  >
    ...
    >
      <!-- Header -->
      <div
        class="flex items-center justify-between pb-2 border-b border-slate-300"
      >
        <div>
          <h1 class="font-bold text-base">Level Rewards</h1>
          <p class="text-sm text-slate-500">
            Free coins and seeds as you level up
          </p>
        </div>
        <button
      ...
      <!-- Progress indicator -->
      <div
        class="flex items-center gap-2 bg-white border-2 border-slate-300 rounded-lg p-2"
      >
        <div class="flex-1">
          <p
            class="font-semibold text-sm text-slate-500 uppercase tracking-wide"
          >
            Current Level
          </p>
        ...
        {#each Array(MAX_LEVEL) as _, i}
          ...
          >
            ...
            <div class="w-10 shrink-0 text-center">
              <p class="font-bold text-sm text-slate-500">Lv.</p>
              <p
                class="font-bold text-base leading-none {special
                  ? 'text-slate-800'
                  : 'text-slate-500'}"
              >
                {level}
            ...
            <!-- Reward icon -->
            <img
              class="w-8 h-8 object-contain shrink-0 {is_claimed
                ? 'grayscale'
                : ''}"
            ...
            <div class="flex-1 min-w-0">
              ...
              {#if special && reward.type === RewardTypes.CROPS}
                <p class="text-sm text-slate-400">Seed reward</p>
              {:else}
                <p class="text-sm text-slate-400">Coins</p>
              {/if}
            </div>
            ...
            <div class="shrink-0">
              {#if is_claimed}
                <span class="text-sm font-semibold text-slate-400">Claimed</span
                >
              {:else if is_unlocked}
                <span class="text-sm font-semibold text-green-700 claim-pulse"
                  >Claim!</span
                >
              {:else}
                <span class="text-sm text-slate-400 font-semibold">Locked</span>
              {/if}
            </div>
          </button>
        {/each}
