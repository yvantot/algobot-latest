<script>
  ...
  let speech = $state(task.description + " Every rule counts, not just the harvest!");
  ...
  function stop(reset=true) {
    ...
    if(reset)world.reset(task.cases[caseIndex]);
...
<div class="live-challenge" role="dialog" aria-modal="true" aria-labelledby="challenge-title" tabindex="-1" bind:this={dialog} onkeydown={keys}>
  ...
  <div class="layout">
    ...
    <section class="program" in:panelIn out:panelOut><div class="program-header"><h2>Robot 0</h2><div><button disabled={running||closing} aria-pressed={mode==="blocks"} onclick={()=>switchMode("blocks")}>Blockly</button><button disabled={running||closing} aria-pressed={mode==="text"} onclick={()=>switchMode("text")}>Text code</button></div></div>
      <div class="goal"><strong>Pass every rule on each test row</strong><ul>{#each rules as rule}<li>{rule.label}</li>{/each}</ul><span>Harvesting everything is not enough if a command fails.</span></div>
      ...
      <details><summary>Commands & rewards</summary><p>bot.right(), bot.left(), bot.harvest(), bot.is_harvestable(), bot.say(value), columns(), rows().</p><ChallengeRewards {task}/><p>One reward per challenge on this farm.</p></details>
