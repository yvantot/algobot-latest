...
<div class="live-challenge" role="dialog" aria-modal="true" aria-labelledby="challenge-title" tabindex="-1" bind:this={dialog} onkeydown={keys}>
  ...
  <div class="layout">
    ...
    <section class="program" in:panelIn out:panelOut><div class="program-header"><h2>Robot 0{task.kind==="team"?" · Grower":""}</h2><div><button disabled={running||closing} aria-pressed={mode==="blocks"} onclick={()=>switchMode("blocks")}>Blockly</button><button disabled={running||closing} aria-pressed={mode==="text"} onclick={()=>switchMode("text")}>Text code</button></div></div>
      ...
      <div class="code-area" class:busy={running}><div class="blockly-host" class:hidden={mode!=="blocks"} bind:this={host}></div><div class="text-host" class:hidden={mode!=="text"}><BotTextEditor bind:value={source} commands={textCommands} readOnly={running||closing} label="Challenge JavaScript program" onEdit={()=>textEdited=true}/></div></div>
      {#if task.kind==="team"}
