...
{#if Modals[ModalTypes.FARM_PERSONALIZE]}
  ...
  >
    ...
    >
      <button
        onclick={handleExit}
        class="absolute right-2 top-0 font-bold text-base">(X)</button
      >
      <div class="p-4 w-full h-full border-b-3 border-[#815831]">
        <h1 class="text-base font-bold text-center">Personalize</h1>
        <p class="text-sm font-bold text-center mb-4">Customize your farm</p>

        <div
          class="flex flex-col gap-2 p-2 border-b-4 border-[#815831] bg-[#c5864b] rounded-lg mb-2"
        >
        ...
        <div
          class="flex flex-col gap-2 p-2 border-b-4 border-[#815831] bg-[#c5864b] rounded-lg mb-4"
        >
          <p class=" text-sm font-bold">Change Farm Name</p>
          <input
            bind:value={new_name}
            class="outline-0 border-t-2 border-[#815831] text-slate-700 bg-slate-100 rounded p-1 px-2"
            placeholder="How about Vegetable Land?"
            type="text"
          />
        </div>
