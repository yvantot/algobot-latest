...
<section class="live-cutscene" aria-label="Command demonstration" transition:fade={{duration:reduced?0:300}}>
 ...
 {#if purchase}<div class="purchase"><p>Try it on this practice farm. Your coins are safe.</p><button onclick={()=>controller?.purchase(purchase.id)}>{purchase.label}</button></div>{/if}
