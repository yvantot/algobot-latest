  ...
  $effect(() => {
    ...
    function startWhenReady() {
      ...
      controller = startLiveDemonstration(update => {
        if (update.chapter !== undefined) { chapter = update.chapter; botLines = {}; }
...
{#if isOpen}
  <div class="live-cutscene" use:dialogFocus tabindex="-1" role="dialog" aria-modal="true" aria-label="Meet your farm" out:fade={{duration:reducedMotion ? 0 : 400}}>
    ...
    <div class="programs" class:multiple={programs.length>1}>
    {#each programs.filter(program=>program.code.some(command=>!command.startsWith("//"))) as program (step.action+program.bot)}
    <aside class="program" out:fly={{x:-30,duration:reducedMotion?0:250,easing:cubicOut}} in:fly={{x:-35,delay:250,duration:reducedMotion?0:400,easing:cubicOut}}>
      <h2>Robot program · Bot {program.bot}</h2>
      <p>{program.role ?? (ready ? "These blocks made it happen" : "Watch the blocks and the farm")}</p>
      <DemonstrationBlocks code={program.code} line={ready?-1:(botLines[program.bot]??-1)} repeat={program.repeat??0} checkHarvest={program.checkHarvest??false}/>
    </aside>
    {/each}
    </div>
...
.programs{position:absolute;top:80px;left:18px;display:grid;gap:12px;width:300px;max-height:62vh;overflow:auto}.programs.multiple{width:auto;right:18px;grid-template-columns:270px 270px;justify-content:space-between;pointer-events:none}.programs.multiple .program{pointer-events:auto}.program{width:100%;flex-shrink:0;background:#f3f4f6;border:3px solid #64748b;border-radius:10px;overflow:hidden;box-shadow:0 5px 15px #0002}.program h2{font-size:18px;font-weight:bold;margin:0;padding:10px 12px;background:#e5e7eb;border-bottom:2px solid #94a3b8}.program>p{font-size:14px;margin:8px 12px;color:#475569}.earnings{position:absolute;left:50%;top:58%;transform:translateX(-50%);padding:12px 20px;border:3px solid #64748b;border-radius:10px;background:#f3f4f6;display:flex;flex-wrap:wrap;align-items:center;gap:8px;font-size:14px;font-weight:bold;color:#166534}.earnings img{width:24px}
...
@media(max-width:900px){.programs{width:240px;top:65px;left:12px}.programs.multiple{width:auto;grid-template-columns:200px 200px;left:12px;right:12px;top:65px}.teacher{padding:12px;gap:12px}.portrait{width:50px}h1{font-size:18px}.speech p{font-size:15px}.live-cutscene{padding:12px}}
@media(max-width:500px){.programs{width:190px;left:12px}.programs.multiple{grid-template-columns:1fr 1fr;gap:6px;right:12px}.programs.multiple h2{font-size:14px}.portrait{display:none}.teacher{max-height:42vh;overflow:auto}footer{flex-wrap:wrap}header span{font-size:14px}button{padding:7px 9px}.speech p{font-size:14px}}
