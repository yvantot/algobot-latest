...
export function assessmentSamples(sessions, assessments, { schema = "10f" } = {}) {
  ...
  for (const a of assessments) {
    ...
    if (schema === COLLECTION_SCHEMA) {
      const active = activeGameplayWindow(snapshots, cutoff);
      if (!active.ready) { reject("requires_20_observed_gameplay_intervals_before_assessment"); continue; }
      x = active.x; inputStart = active.start; inputEnd = active.end;
    } else {
      ...
      inputStart = (schema === RESEARCH_SCHEMA ? recentWindow : window)[0].timestamp_ms;
      inputEnd = window.at(-1).timestamp_ms;
    }
    samples.push({ source_type: "recorded", label_source: "independent_scored_task",
      ...
      input_start_ms: inputStart, input_end_ms: inputEnd,
      assessment_start_ms: cutoff, real_timesteps: 20, y: a.score / a.max_score,
      x });
  }
  return { samples, excluded, feature_names: researchFeatureNames(schema) ?? FEATURE_NAMES, feature_schema: schema,
    target: "Independent task score / maximum; not the legacy gameplay proxy",
    deployment_ready: false };
}

export function challengeSamples(sessions, taskId = "ready-row-v3", { schema = sessions.some(s => s.research_features?.schema === COLLECTION_SCHEMA) ? COLLECTION_SCHEMA : RESEARCH_SCHEMA } = {}) {
  ...
  const result = assessmentSamples(sessions, assessments, { schema });
