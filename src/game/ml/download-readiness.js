...
export function downloadReadiness(session, { cleared = false, sessions = [session] } = {}) {
  ...
  const count = challengeSamples(participantSessions, "first-harvest-v1").samples.length;
  ...
  if (!attempts.length) return { ready:false, message:'Complete "Your first harvest" in Challenges before downloading. Run your program and wait for its score. A low score is okay! If Challenges is not available yet, keep playing until it unlocks.' };
