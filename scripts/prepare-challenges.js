...
const [input, output, taskId = "ready-row-v2"] = process.argv.slice(2);
if (!input || !output || !CHALLENGES.some(task => task.id === taskId)) {
  throw Error("Usage: node scripts/prepare-challenges.js <export-file-or-folder> <NEW-output.json> [ready-row-v2|changing-row-v2]");
}
