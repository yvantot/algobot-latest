...
A juvenile flame dies when its crop is gone. An adult without fuel dies immediately if no unlit adjacent crop is available; otherwise it has at most one spread interval before dying. A crop that spoils while burning is also removed completely. Fire does not consume soil moisture.

## Rain rules

Each cloud enters horizontally from one side, travels for 1.5 game seconds, stops above its selected tile, rains for 6 seconds, then leaves over 1.2 seconds. Drops appear every 0.6 seconds and take 0.45 seconds to hit soil. Soil changes on impact, not when the event banner appears.
...
| Fire stage / damage / spread interval | 3 seconds | 1 second |
| Fire damage before crop resistance | 1 per interval | 8 per interval |
...
The requested PNGs were not present in this checkout during implementation. Simple inline SVG placeholders keep the feature playable without missing-file crashes. The artwork resolver automatically prefers the exact supplied PNG filenames when added under `public/sprites`, `public/assets`, `assets`, or `src/assets`; restart Vite/rebuild after adding them.

Required names: `icon_cloud.png`, `icon_raindrop.png`, `icon_fire_0.png`, `icon_fire_1.png`, `icon_fire_2.png`, `icon_smoke_0.png`, `icon_smoke_1.png`. `public/sprites` is the preferred location. The fallback graphics are temporary, not a claim that the supplied art was found.
