...
4. After the tutorial, challenges appear only after a usable gameplay window is ready: roughly **110 seconds of uninterrupted normal-speed gameplay**. A long pause, hidden tab or speed change can require another observation window. In Dev Console's DDA / Research Data area, press **Check Collection**. Look for “Gameplay window ready for a first challenge attempt.” This is a snapshot; recheck if conditions change.
5. Have the student attempt the agreed challenge using the same assistance rules as everyone else. Students may Stop & Edit within the same attempt. Every stop is retained. The first fully evaluated submission is the target, including failure or zero; later evaluated retries cannot replace it. Closing before a scored submission leaves no usable target for this task. Do not mark unfinished work as zero or reset exposure history to manufacture a new first attempt.
