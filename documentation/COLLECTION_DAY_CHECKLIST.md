...
3. Let the student complete the tutorial normally. Do not use tutorial-skip, resource grants, forced events or other developer gameplay controls: they mark the session as a developer test, excluded from training. Opening Dev Console, Check Collection and downloading JSON are read-only research actions.
4. After the tutorial, allow roughly **110 seconds of uninterrupted normal-speed gameplay before opening the chosen challenge**. A long pause, hidden tab or speed change can require another observation window. In Dev Console's DDA / Research Data area, press **Check Collection**. Look for “Gameplay window ready for a first challenge attempt.” This is a snapshot; recheck if conditions change.
5. Have the student attempt the agreed challenge using the same assistance rules as everyone else. The first submitted program's result is the target, including a valid zero score. Retrying is allowed for learning, but does not replace that first score. Closing before a scored submission, stopping the first run, or opening too early can leave no usable target for this task. Do not mark unfinished work as zero or reset exposure history to manufacture a new first attempt.
