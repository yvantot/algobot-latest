...
9. Export **All Sessions JSON** after each player and retain the original download. Check storage warnings. Browser storage is a checkpoint, not a server or guaranteed backup; losing power can lose changes since the last successful 30-second save. Never clear storage before verifying the downloaded file.
...
node scripts/train-lstm-experiment.js training/experiments/NEW-run path/to/NEW-samples.json
```
...
The importer writes accepted samples plus an exclusion report and input hashes. It refuses to overwrite an existing output. Every accepted window has 20 actual consecutive normal-speed gameplay snapshots, all strictly before assessment start. The training command uses recorded independent scores only; synthetic labels derived from the old formula are not mixed into this target. At least three distinct participant IDs are required mechanically, but that is not evidence of an adequate sample size.
...
The current importer is deliberately conservative and still uses the legacy ten inputs. Uncapped counters and raw events allow a later recent-window feature set without recollecting play. Evaluate that change on training/validation participants first, version the schema, then implement the same transformation at runtime. New weights are candidates only and are not automatically deployed.
